class EndorsementPosController < ApplicationController
  ...
  def index
    ...
    if params[:po_number]
      ...
      # DEFAULT COLLECTION
      query_results = PurchaseOrder.where('endorsement_po_id IS NOT NULL AND endorsement_po_id > 0').order(po_number: :desc)
    end
  ...
  def get_project_pos
    ...
    proj.purchase_orders.order(:po_number).each do |po|
      pos << {
        ...
        selected: false,
        endorsed_po: po.endorsement_po_id.to_i > 0 ? true : false
      }
    end
